{foreach from=$mentions item=item}
<div id="sina-mention-{$item['id']}" username="{$item['user']['screen_name']}">
	<div class="single">
		...
		<div class="single-operate">
			<span onClick="gui.addFavourite('sina','mention','{$item['id']}')">收藏</span> | 
			<span onClick="gui.openRetweet('sina','mention','{$item['id']}'
			{if $item['retweeted_status']},true{/if}
			)">转发{if {$item['rt_count']}}({$item['rt_count']}){/if}</span> | 
			<span onClick="gui.openComments('sina','mention','{$item['id']}')">评论{if {$item['comments_count']}}({$item['comments_count']}){/if}</span>
		</div>
		<br class="clean"/>
	</div>
</div>
